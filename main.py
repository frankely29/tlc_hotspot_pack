...
# BEST endpoint for your GitHub Pages map (no download prompt)
...
# Keep your old download endpoint too (forces download)
@app.get("/download")
def download():
    if not OUT_PATH.exists():
        return JSONResponse(
    ...
    return FileResponse(
        str(OUT_PATH), media_type="application/json", filename="hotspots_20min.json"
    )
